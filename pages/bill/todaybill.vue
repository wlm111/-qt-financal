<template>
	<view>
		<!-- 今日统计 -->
		<bill-data :homedata="todaydata"></bill-data>
	<swiper class="swiper-box" :current="tabIndex" :style="{height:swiperheight+'px'}" >
		<swiper-item>
			<scroll-view scroll-y class="list">
				<!-- 列表 -->
				<block @changeevent="updateData" v-for="(item,index) in todayrecord" :key="index">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取今日账单列表
			async gettodayList(){
				let [err,res] = await this.$http.get(`/user/todayrecord`,{},{ token:true });
				if (!this.$http.errorCheck(err,res)) return;
